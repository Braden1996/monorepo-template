...
module.exports = (workspacePath, extraWorkspaceAliasesForPackages) => {
  return async (env, argv) => {
    ...
    const aliases = getWorkspaceAliasFixes(workspacePath, ['react', 'react-native-web', ...extraWorkspaceAliasesForPackages]);

    const expoAliasEntries = Object.entries(config.resolve.alias).filter(([key]) => !skipImports.includes(key.split('$')[0]))
    const expoAliasPackages = expoAliasEntries.map(([_, package]) => package)
    const expoAliasPackagesResolved = getWorkspaceAliasFixes(workspacePath, expoAliasPackages);
    const expoAliases = Object.entries(expoAliasPackagesResolved).reduce((acc, [package, resolution]) => ({
      ...acc,
      [expoAliasEntries.find(([_, p]) => p === package)[0]]: resolution
    }), {})

    config.resolve.alias = { ...aliases, ...expoAliases, 'react-native': expoAliases['react-native$'] }
